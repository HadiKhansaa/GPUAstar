...
// Create a constuctor that takes height and width as arguments


Pathway::Pathway(int grid_height, int grid_width)
{
    // if (!vm_options.count("width") || !vm_options.count("height")) {
    //     cout << "Please set the width and height for your graph." << endl
    //         << "===============================================" << endl
...
void generateRandomGridWithPath(int n, int m) {
    vector<vector<int>> grid(n, vector<int>(m, 0));
    srand(time(0));

    // Ensure there's a path from (0, 0) to (n-1, m-1)
    int x = 0, y = 0;
    grid[x][y] = 1;
    
    while (x < n - 1 || y < m - 1) {
        int direction = rand() % 2;
        if (direction == 0 && x < n - 1) {
            x++;
        } else if (y < m - 1) {
            y++;
        }
    ...
    // Randomly fill the rest of the grid with 1s and 0s
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (grid[i][j] != 1) {
                grid[i][j] = rand() % 2;
